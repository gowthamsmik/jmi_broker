...
if ($user) {
    ...
    // Fetch live accounts
    $stmtAccounts = $conn->prepare("SELECT id, status, account_id,'JMI Server',account_type,currency,account_group,password,investor_password,'Ameer',leverage FROM fx_accounts WHERE website_accounts_id = ? and account_radio_type=1");
    $stmtAccounts->bind_param("i", $userId);
