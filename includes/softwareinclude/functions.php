...
function sendMailForDocumentUpload($mailBody,$mailSubject){
    ...
    $mailsBody=documentUploadTemple($mailBody);
    // $message = (new Swift_Message(''))
    //         ->setFrom(['marketing@jmibrokers.com' => 'Jmi brokers'])
    //         ->setTo($mailTo)
    //         ->setBody($mailsBody,'text/html')
    //         ->setSubject($mailSubject);
        ...
        // // Send the email and check for success
        // $mailSent = $mailer->send($message);
        $mailSent = generalSupportEmail($mailsBody, $mailSubject, $mailTo,'');
...
function generalSupportEmail($mailBody, $mailSubject, $mailTo, $attachment)
{
    try{
    global $supporterMessage;
    global $supporterMailer;
    $supporterMessage->setTo($mailTo)
        ->setBody($mailBody, 'text/html')
        ->setSubject($mailSubject);
    if ($attachment) {
        $attachedFile = Swift_Attachment::fromPath($attachment);
        $supporterMessage->attach($attachedFile);
    }
    return  $supporterMailer->send($supporterMessage);
} catch (Exception) {
    return '';
}
...
function generalMarketingEmail($mailBody, $mailSubject, $mailTo,$attachment)
{
    try{
    global $marketingMailer;
    global $marketingMessage;
    $marketingMessage->setTo($mailTo)
        ->setBody($mailBody, 'text/html')
        ->setSubject($mailSubject);
    if ($attachment) {
        $attachedFile = Swift_Attachment::fromPath($attachment);
        $marketingMessage->attach($attachedFile);
    }
    return $marketingMailer->send($marketingMessage);
} catch (Exception) {
    return '';
}
